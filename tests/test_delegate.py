...
def test_delegate_success(alice, bot, resource, registry):
    registry.add_claim(RightsClaim(alice, resource, can_read=True, can_write=True, can_delegate=True))
...
def test_delegate_write_success(alice, bot, resource, registry):
    registry.add_claim(RightsClaim(alice, resource, can_read=True, can_write=True, can_delegate=True))
    registry.delegate(RightsClaim(bot, resource, can_read=True, can_write=True), delegated_by=alice)
    ok, _, _ = registry.can_act(bot, resource, "write")
    assert ok


def test_delegate_reduced_confidence(alice, bot, resource, registry):
    registry.add_claim(RightsClaim(alice, resource, can_read=True, can_delegate=True, confidence=0.9))
    registry.delegate(
...
def test_cannot_delegate_write_without_write(alice, bot, resource, registry):
    registry.add_claim(RightsClaim(alice, resource, can_read=True, can_write=False, can_delegate=True))
    with pytest.raises(PermissionError, match="write"):
        registry.delegate(RightsClaim(bot, resource, can_read=True, can_write=True), delegated_by=alice)


def test_cannot_delegate_read_without_read(alice, bot, resource, registry):
    registry.add_claim(RightsClaim(alice, resource, can_read=False, can_write=True, can_delegate=True))
...
def test_cannot_subdelegate_without_can_delegate(alice, bot, resource, registry):
    ...
    registry.delegate(RightsClaim(bot, resource, can_read=True, can_delegate=False), delegated_by=alice)
...
def test_confidence_cannot_exceed_delegator(alice, bot, resource, registry):
    registry.add_claim(RightsClaim(alice, resource, can_read=True, can_delegate=True, confidence=0.8))
...
def test_confidence_equal_to_delegator_is_ok(alice, bot, resource, registry):
    registry.add_claim(RightsClaim(alice, resource, can_read=True, can_delegate=True, confidence=0.8))
    registry.delegate(RightsClaim(bot, resource, can_read=True, confidence=0.8), delegated_by=alice)
    ok, conf, _ = registry.can_act(bot, resource, "read")
    assert ok

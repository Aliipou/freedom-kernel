...
from freedom_theory.kernel.entities import (
    AgentType, ConsentRecord, Entity, Resource, ResourceType
)
...
class FreedomVerifier:
    ...
    def verify(self, action: Action) -> VerificationResult:
        ...
        for resource in action.resources_delegate:
            if resource.rtype == ResourceType.PERSON:
                violations.append(
                    f"DELEGATION DENIED on {resource}: PERSON resources cannot be delegated (A2)"
                )
        ...
        # Consent check: cross-human resource access requires ConsentRecord (A3)
        all_touched = list(action.resources_read) + list(action.resources_write)
        excluded: set[Entity] = {action.actor}
        if action.actor.is_machine():
            owner = self.registry.owner_of(action.actor)
            if owner is not None:
                excluded.add(owner)
        for resource in all_touched:
            for human in self.registry.human_claimants_for(resource):
                if human in excluded:
                    continue
                relevant = [cr for cr in action.consent_records if cr.grantor == human]
                if not relevant:
                    warnings.append(
                        f"No ConsentRecord from {human.name} for accessing {resource}. "
                        f"Cross-human resource access without explicit consent."
                    )
                elif not any(cr.is_valid() for cr in relevant):
                    violations.append(
                        f"ConsentRecord from {human.name} for {resource} has expired. "
